...
const LazyLoadCard = ({ card, index, hoverIndex, setHoverIndex, navigate }) => {
  const { ref, inView } = useInView({
    triggerOnce: true, // Load only once when the card comes into the viewport
  ...
  return (
    <Card
      ...
      sx={{
        height: { xs: '300px', sm: '420px' }, // Adjust height for different screen sizes
        display: 'flex',
        flexDirection: 'column',
        justifyContent: 'space-between',
        backgroundImage: inView ? `url(${card.backgroundImage})` : 'none',
    ...
    >
      ...
      >
        <Typography
          variant="h5"
          component="h2"
          sx={{ fontFamily: "'Arapey', serif", color: 'white', zIndex: 2, fontSize: '1.75rem', padding: '0.1em 0.4em' }}
        >
          {card.title}
        </Typography>
